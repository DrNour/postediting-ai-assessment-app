...
def get_adaptive_translation_help(source_text, student_draft, help_type, student_question=''):
    """Return on-demand Gemini assistance for the adaptive-translation condition.
    ...
    """
    api_key = safe_text(st.secrets.get("GEMINI_API_KEY", ""))
    if not api_key:
    ...
    try:
        ...
        return None, "Gemini support is not installed. Add google-genai>=1 to requirements.txt and redeploy the app."
...
def autosave_student_draft(assignment_id, task_type, student_id_key, student_name_key, answer_key):
    """Persist the current response when the student leaves the response box.

    Streamlit reruns can happen during ordinary interaction. Saving on text-area
    change means a browser refresh or temporary reconnect is much less likely to
    erase the student's latest completed editing burst.
    """
    student_id = safe_text(st.session_state.get(student_id_key, ""))
    if not student_id:
        return
    draft_text = safe_text(st.session_state.get(answer_key, ""))
    if not draft_text:
        return
    try:
        result = save_student_draft(
            {
                ...
                "student_name": safe_text(st.session_state.get(student_name_key, "")),
                "draft_text": draft_text,
            }
        )
        if result is not None:
            st.session_state[f"autosave_notice_{assignment_id}_{normalize_task_type(task_type)}"] = True
    except Exception:
        # Autosave should never interrupt typing. Manual save remains available.
        pass
...
def student_assignment_page():
    ...
    student_id_key = f"student_id_{selected_assignment_id}"
    student_name_key = f"student_name_{selected_assignment_id}"
    ...
    st.caption(
        "Drafts are stored in Supabase and kept separate for No-AI Translation, Adaptive Translation, and Post-editing. "
    ...
    if is_translation(task_type):
        with work_col:
            ...
            student_answer = st.text_area(
                ...
                label_visibility="collapsed",
                on_change=autosave_student_draft,
                args=(
    ...
    autosave_notice_key = f"autosave_notice_{selected_assignment_id}_{normalize_task_type(task_type)}"
    if st.session_state.pop(autosave_notice_key, False):
